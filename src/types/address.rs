...
impl Address {
    pub fn from_public_key_bytes(bytes: &[u8]) -> Address {
        // It uses SHA256 (from **ring** crate) to hash the input bytes,
        // and takes the last 20 bytes and convert them into a __Address__ struct.
        let hash = digest::digest(&digest::SHA256, bytes);
        let mut buffer: [u8; 20] = [0; 20];
        buffer.copy_from_slice(&hash.as_ref()[12..32]);
        Address(buffer)
    }
}
